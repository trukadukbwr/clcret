...
if class ~= "PRIEST" then return end -- change to correct class!!!

local _, xmod = ...
xmod = xmod.disciplinepriestmodule -- change to spec module!!!
...
local function Set(info, val)
	...
	if info[#info] == "disciplineprio" then -- change genericprio to specprio!!!
		xmod.Update()
	end
end
...
function xmod.BuildOptions()
	...
	return {
		...
		type = "group", 
		childGroups = "tab", 
		name = "Discipline Priest", -- change Generic to Spec!!!
		args = {
			tabPriority = {
			order = 1, 
			type = "group", 
					...
					args = {
						info = {
							...
							normalPrio = {
								...
								args = {
									disciplineprio = { -- change genericprio to specprio!!!
										order = 2, 
										type = "input", 
										width = "full", 
										name = "",
