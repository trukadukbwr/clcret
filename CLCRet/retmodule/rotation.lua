...
xmod.defaults = {
	...
	prio = "dh tv_dh_2 tv_4pc fr hol es woa tv5 ds tvdp ts2_exp tv ts1 j_bell how_anshe dt boj2 boj j how ts2 cs2 cs",
	rangePerSkill = false,
...
local actions = {
	...
	j = {
		...
		GetCD = function()
			...
			knownDH = IsPlayerSpell(idDivineHammer)
			AssistedDS = select(1, C_AssistedCombat.GetNextCastSpell())
	...
	boj = {
		...
		local cd, charges = GetBoJData()
			...
			if (s1 ~= idBladeOfJustice) and knownBOJ and (GetCooldown(idBladeOfJustice) < 1) and DivineHammerCheck and not s_buff_AllIn then
	...
	boj2 = {
		...
		local cd, charges = GetBoJData()
			...
			if (s1 ~= idBladeOfJustice) and (s_BoJCharges == 2) and (s_hp <= 3) and knownBOJ and (GetCooldown(idBladeOfJustice) < 1) and DivineHammerCheck and not s_buff_AllIn then
				return GetCooldown(idBladeOfJustice)
			end
	...
	-- ds = {
		...
		-- GetCD = function()
			...
			-- AssistedDS = select(1, C_AssistedCombat.GetNextCastSpell())
			-- aoeCheck = ((AssistedDS == 53385) and db.aoeMode)
			
			-- if (s1 ~= idDivineStorm) and ((s_hp > 2) or s_buff_DivinePurpose) and not(knownHOL) and (aoeCheck or s_buff_EmpyreanPowerDS) then
				-- return 0
			-- end
			
			-- if (s1 ~= idDivineStorm) and (s_hp > 2) and not(knownHOL) and not db.aoeMode then
				-- return 100
			-- end
	...
	tv = {
		...
		GetCD = function()
			...
			AssistedDS = select(1, C_AssistedCombat.GetNextCastSpell())
			-- assistedDS = need aoe / db.aoemode = suggest aoe
			aoeCheck = ((not(AssistedDS == 53385) and db.aoeMode) or not db.aoeMode)
	...
	tv_dh = {
		...
		info = "Templar's Verdict with Divine Hammer active",
	},
	...
	tv_dh_2 = {
		...
		GetCD = function()
			...
			cooldownInfo = C_Spell.GetSpellCooldown(198034)
		...
		info = "Templar's Verdict with < 2 secs left on Divine Hammer",
	},
	...
	tv_ds = {
		...
		GetCD = function()
			...
			if (s1 ~= idTemplarsVerdict) and knownTV and ((s_hp > 2) or s_buff_DivinePurpose) and s_buff_EmpyreanLegacyTV and not(knownHOL) and not(AssistedDS == 53385) then
				return 0
			end
...
function xmod.Rotation()
	...
	if (AssistedDS == 53385) and db.aoeMode then
		s1 = idDivineStorm
	end
