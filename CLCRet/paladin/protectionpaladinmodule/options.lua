...
local _, xmod = ...
xmod = xmod.protectionpaladinmodule -- change to spec module!!!
...
local function Set(info, val)
	...
	if info[#info] == "protpallyprio" then -- change genericprio to specprio!!!
		xmod.Update()
	end
end
...
function xmod.BuildOptions()
	...
	return {
		...
		type = "group", 
		childGroups = "tab", 
		name = "Protection Paladin", -- change Generic to Spec!!!
		args = {
			tabPriority = {
			order = 1, 
			type = "group", 
					...
					args = {
						info = {
							...
							normalPrio = {
								...
								args = {
									protpallyprio = { -- change genericprio to specprio!!!
										order = 2, 
										type = "input", 
										width = "full", 
										name = "",
