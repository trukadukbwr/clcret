...
if class ~= "SHAMAN" then return end -- change to correct class!!!

local _, xmod = ...
xmod = xmod.restorationshamanmodule -- change to spec module!!!
...
local function Set(info, val)
	...
	if info[#info] == "restorationshamanprio" then -- change genericprio to specprio!!!
		xmod.Update()
	end
end
...
function xmod.BuildOptions()
	...
	return {
		...
		type = "group", 
		childGroups = "tab", 
		name = "Restoration Shaman", -- change Generic to Spec!!!
		args = {
			tabPriority = {
			order = 1, 
			type = "group", 
					...
					args = {
						info = {
							...
							normalPrio = {
								...
								args = {
									restorationshamanprio = { -- change genericprio to specprio!!!
										order = 2, 
										type = "input", 
										width = "full", 
										name = "",
