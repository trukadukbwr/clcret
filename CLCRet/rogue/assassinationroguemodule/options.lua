...
if class ~= "ROGUE" then return end -- change to correct class!!!

local _, xmod = ...
xmod = xmod.assassinationroguemodule -- change to spec module!!!
...
local function Set(info, val)
	...
	if info[#info] == "assassinationprio" then -- change genericprio to specprio!!!
		xmod.Update()
	end
end
...
function xmod.BuildOptions()
	...
	return {
		...
		type = "group", 
		childGroups = "tab", 
		name = "Assassination Rogue", -- change Generic to Spec!!!
		args = {
			tabPriority = {
			order = 1, 
			type = "group", 
					...
					args = {
						info = {
							...
							normalPrio = {
								...
								args = {
									assassinationprio = { -- change genericprio to specprio!!!
										order = 2, 
										type = "input", 
										width = "full", 
										name = "",
